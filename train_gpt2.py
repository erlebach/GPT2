...
class GPT(nn.Module):
    ...
    def configure_optimizers(self, weight_decay, learning_rate, device):
        ...
        print(f"configuring optimizers with fused={use_fused} (fused_available={fused_available}) on device {device}")
...
def train():
    ...
    def get_lr(it):
        ...
        coeff = 0.5 * (1.0 + math.cos(math.pi * decay_ratio)) #coeff starts at 1 and then goes to 0 in a cosine like fashion
    ...
    for step in range(max_steps):
        ...
        t1 = time.time()
        dt = (t1 - t0)*1000
        print(f"step: {step:4d}, loss: {loss_accum.item()}, norm: {norm:.4f}, dt: {dt:.2f}ms")
